...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Trabalhando nisso, aguarde por favor☺️',
				success: 'Finalizado com sucesso',
				error: {
					stick: 'Algo deu errado😪, tente novamente ',
					Iv: 'Link incorreto 😪'
				},
				only: {
					group: 'Esse comando só pode ser usado em grupos 😪' ,
					ownerG: 'Só admins podem usar isso espertinho 🤭 ',
					ownerB: 'Só o dono pode usar este comando :-: , 
					admin: 'Só o dono pode:-:,
					Badmin: 'Não sou admin 😐'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511963001592@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'yt2mp3':
					if (args.length < 1) return reply('Inclua o link😐')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('Diga o que procura após o ytsearch')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Inclua o nome no comando ', text, {quoted: mek})
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('O que deseja escrever? ')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Inclua o link😐')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Insira o texto desejado')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'bc':
					if (!isOwner) return reply('Quem és tu? ') 
					...
					if (mentioned.length > 1) {
						...
						mentions(`PROMOVEU @${mentioned[0].split('@')[0]} a Admin🤭`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Deseja adicionar quem? ') 
					if (args[0].startsWith('08')) return reply('Insira um número válido')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Esse número é privado, não foi possível adicionar, desculpe 😶' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem quer expulsar ')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Kickado com sucesso:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Kickado : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
